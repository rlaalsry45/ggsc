...
<div class="wrap"> 
	...
	<div class="container">
		...
		<form commandName="searchVO" id="admTrstFrm" name="admTrstFrm" method="post">
		<!-- content --> 
		<div class="content"> 
			<div class="paT40 paB25">
				<h1>금전신탁계약서 첨부파일 다운로드</h1>
			</div>
			...
			<c:forEach var="result" items="${trstFileList}" varStatus="status">
			
				
					<tr>
						<td><c:out value="${result.trstFileNum}"/></td>
						...
						<td>
							<c:if test="${result.orgFileNm != null}">
								<a href="javascript:fn_download('<c:out value="${result.trstFileNum}"/>','<c:out value="${result.orgFileNm}"/>','<c:out value="${result.sysFileNm}"/>','<c:out value="${result.filePath}"/>','<c:out value="${result.fileSize}"/>');" class="btn_big"><strong>다운로드</strong></a>
							</c:if>
						</td> 
					</tr>
			</c:forEach>
...
<!-- /wrap -->  
    ...
    <script type="text/javaScript" language="javascript" defer="defer">
	    ...
	    function fn_sch() {
	       	...
	       	document.admTrstFrm.action = "/admTrstFileList.do";
	       	document.admTrstFrm.submit();
	    ...
	    /* 파일다운로드 */
	     function fn_download(trstFileNum, orgFileNm, sysFileNm, filePath, fileSize) {
	    	 document.admTrstFrm.tmpSys_file_nm.value = sysFileNm;
			document.admTrstFrm.tmpOrg_file_nm.value = orgFileNm;
			document.admTrstFrm.tmpFile_path.value = filePath;
			document.admTrstFrm.tmpFile_size.value = fileSize;
			
			document.admTrstFrm.action = "/admTrstFileDwn.do";
	       	document.admTrstFrm.submit();
