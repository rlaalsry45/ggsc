...
<%@ taglib prefix="spring" uri="http://www.springframework.org/tags"%>
	            ...
	            <div class="row">
					<div class="col-md-12 col-sm-12">
						...
						<table class="tbl_type" border="1" cellspacing="0" summary="공지사항리스트"> 
							...
							<input type="hidden" id="tmpRpt2_name" name="tmpRpt2_name" value="" />
							...
							<c:forEach var="result" items="${cdList}" varStatus="status">
								...
								<tr>
									<td>${status.count}</td> 
									<td  style="cursor:pointer;" onclick="javascript:fn_detail('<c:out value="${result.mngNo}"/>','<c:out value="${result.witrId}"/>','<c:out value="${result.witrName}"/>','<c:out value="${result.rpt2Id}"/>','<c:out value="${result.rpt2Name}"/>');" onmouseover="this.style.backgroundColor='#f1f1f1'" onmouseout="this.style.backgroundColor='#ffffff'">
										<c:out value="${result.mngNo}"/>
									</td> 
									<td><c:out value="${result.witrName}"/></td> 
									<td><c:out value="${result.rpt2Name}"/></td> 
									<td>
										<a href="javascript:fn_file_download('<c:out value="${result.mngNo}"/>','<c:out value="${result.witrId}"/>','<c:out value="${result.rpt2Id}"/>','<c:out value="${result.sysFileNm}"/>','<c:out value="${result.orgFileNm}"/>','<c:out value="${result.filePath}"/>','<c:out value="${result.fileSize}"/>');" ><strong>${result.orgFileNm}</strong></a>
									</td>
								</tr> 
							</c:forEach>
    ...
    <script type="text/javaScript" language="javascript" defer="defer">
	    ...
	    //상세보기 화면으로 이동
	    function fn_detail(mngNo, witrId,witrName, rpt2Id,rpt2Name) {
			...
			document.mypageFrm.tmpRpt2_id.value = rpt2Id;
			document.mypageFrm.tmpRpt2_name.value = rpt2Name;
	       	document.mypageFrm.action = "/mypageReportUploadMove.do";
    	...
    	//보고서 다운로드
	    function fn_file_download(mng_no, witr_id, rpt2_id, sys_file_nm, org_file_nm, file_path,file_size) {
			
			
			document.mypageFrm.tmpMng_no.value = mng_no;
			document.mypageFrm.tmpWitr_id.value = witr_id;
			document.mypageFrm.tmpRpt2_id.value = rpt2_id;
			...
			document.mypageFrm.action = "/mypageReportDownLoad.do";
	       	document.mypageFrm.submit();
