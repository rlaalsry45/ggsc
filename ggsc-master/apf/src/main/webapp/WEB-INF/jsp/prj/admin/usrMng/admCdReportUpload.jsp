...
<%@ taglib prefix="spring" uri="http://www.springframework.org/tags"%>
		...
		<div class="container">
			...
			<div class="content"> 
				<div class="paT40 paB25">
					<h1>사전의료요양지시서 감독기록 업로드</h1>
				</div>
				
				<h3 class="title"><b>감독기록 다운로드</b></h3>
				<form id="admCdDownFrm" name="admCdDownFrm" method="post" enctype="multipart/form-data">
					...
					<input type="hidden" id="tmpRpt2_id" name="tmpRpt2_id" value="" />
					<input type="hidden" id="tmpRpt2_num" name="tmpRpt2_num" value="" />
					...
					<c:forEach var="result" items="${cdReportList}" varStatus="status">
							<tr>
								<td>${status.count}</td> 
								...
								<td>
									<a href="javascript:fn_file_download('<c:out value="${result.mngNo}"/>','<c:out value="${result.witrId}"/>','<c:out value="${result.rpt2Id}"/>','<c:out value="${result.rpt2Num}"/>','<c:out value="${result.sysFileNm}"/>','<c:out value="${result.orgFileNm}"/>','<c:out value="${result.filePath}"/>','<c:out value="${result.fileSize}"/>');" class="btn_big"><strong>${result.orgFileNm}</strong></a>
								</td>
								<td>
									<a href="javascript:fn_delete('<c:out value="${result.mngNo}"/>','<c:out value="${result.witrId}"/>','<c:out value="${result.rpt2Id}"/>','<c:out value="${result.rpt2Num}"/>');" style="width:60px;">삭제하기</a>
								</td>
							</tr>
					</c:forEach>
				...
				<div class="form_table">
					<form id="admCdFrm" name="admCdFrm" method="post" enctype="multipart/form-data">
							<table class="notice_view" border="1" cellspacing="0" summary=""> 
								...
								<input type="hidden" id="rpt2_num" name="rpt2_num" value="${usrMngVO.rpt2_num}" >
    ...
    <script type="text/javaScript" language="javascript" defer="defer">
	    ...
	    function fn_upload() {
			...
			document.admCdFrm.action = "/admCdReportUpload.do";
	       	document.admCdFrm.submit();
	    }
	    
	    
		function fn_file_download(mng_no, witr_id, rpt2_id, rpt2_num,sys_file_nm, org_file_nm, file_path,file_size) {
			
			
			document.admCdDownFrm.tmpMng_no.value = mng_no;
			document.admCdDownFrm.tmpWitr_id.value = witr_id;
			document.admCdDownFrm.tmpRpt2_id.value = rpt2_id;
			document.admCdDownFrm.tmpRpt2_num.value = rpt2_num;
	  ...
	  //보고서 삭제
	    function fn_delete(mngNo, witrId, rpt2Id, rpt2Num) {
	    	
	    	var data=new FormData();
	    	data.append("mng_no",mngNo);
	    	data.append("witr_id",witrId);
	    	data.append("rpt2_id",rpt2Id);
	    	data.append("rpt2_num",rpt2Num);
	    	
	    	$.ajax({
		        data: data ,
		        type: "POST",
		        url: "/admCdReportDel.do",
		        cache: false,
		        contentType: false,
		        processData: false,
		        dataType:"json",	        
