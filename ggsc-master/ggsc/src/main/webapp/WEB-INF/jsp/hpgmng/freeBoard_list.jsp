...
<script type="text/javascript">
	...
	function fn_detail(num){
		document.location.href = "/gnoincoundb/freeBoardDtl.do?num=" + num + "&mnuCd=${mnuCd}";
	}

	function fn_search(mnuCd) {
		document.searchForm.action = "/gnoincoundb/freeBoardList.do?mnuCd=" + mnuCd;
	...
	<h2 class="h2-title"><i class="fa fa-check-square"></i>게시판관리</h2>
		...
		<form name="searchForm" id="searchForm" method="get" onsubmit="return false">
			...
			<div class="search-box">
				<div class="search-group">
					<span style="padding-left: 100px;">등록일</span> <input type="text" name="schFblStartDate" id="datepicker1" value="${vo.schFblStartDate}" style="width: 100px;" readonly/> &nbsp;&nbsp;&nbsp; ~ &nbsp;<input type="text" name="schFblEndDate" id="datepicker2" value="${vo.schFblEndDate}" style="width: 100px;"readonly />
					<span class="label">
						<select id="schFblType" name="schFblType" style="margin-left: 20px;">
							<option value="1" <c:if test="${vo.schFblType==1}">selected</c:if>>공지제목</option>
							<option value="2" <c:if test="${vo.schFblType==2}">selected</c:if>>작성자</option>
							<option value="3" <c:if test="${vo.schFblType==3}">selected</c:if>>내용</option>
						</select>
					</span>
					<span class="form" style="margin-left: 25px;"><input type="text" name="schFblText" id="schFblText" value="${vo.schFblText }" enterSearch data-button='#searchBtn'/></span>
				</div>
				<div class="btn">
		...
		<form name="frm" id="frm" method="post" onsubmit="return false">
			...
			<table class="table-style1" style="margin-bottom: 5px;">
				...
				<tbody id="tby1">
					<c:if test="${freeBrdList.size() == 0 }">
						<tr>
							<td colspan="5">정보가 없습니다.</td>
						</tr>
					</c:if>
					<c:if test="${freeBrdList.size() > 0 }">
						<c:forEach items="${freeBrdList }" var="result">
							<tr>
								<td onclick="javascript:fn_detail('${result.num}')">${result.rnum }</td>
								<td style='text-indent:10px; text-align:left' onclick="javascript:fn_detail('${result.num}')">
									<c:choose>
