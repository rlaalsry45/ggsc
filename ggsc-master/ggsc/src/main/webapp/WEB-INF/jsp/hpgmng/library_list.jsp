...
<script type="text/javascript">
	...
	function fn_detail(num){
		document.location.href = "/gnoincoundb/libraryDtl.do?num=" + num + "&mnuCd=${mnuCd}";
	}

	function fn_search(mnuCd) {
		document.searchForm.action = "/gnoincoundb/libraryList.do?mnuCd=" + mnuCd;
...
<section id="content">
	...
	<h2 class="h2-title"><i class="fa fa-check-square"></i>문서자료실</h2>
		...
		<form name="searchForm" id="searchForm" method="get" onsubmit="return false">
			...
			<div class="search-box">
				<div class="search-group">
					<span style="padding-left: 100px;">등록일</span> <input type="text" name="schDcuStartDate" id="datepicker1" value="${vo.schDcuStartDate}" style="width: 100px;" readonly/> &nbsp;&nbsp;&nbsp; ~ &nbsp;<input type="text" name="schDcuEndDate" id="datepicker2" value="${vo.schDcuEndDate}" style="width: 100px;" readonly/>
					<span class="label">
						<select id="schDcuType" name="schDcuType" style="margin-left: 20px;">
							<option value="1" <c:if test="${vo.schDcuType==1}">selected</c:if>>공지제목</option>
							<option value="2" <c:if test="${vo.schDcuType==2}">selected</c:if>>작성자</option>
							<option value="3" <c:if test="${vo.schDcuType==3}">selected</c:if>>내용</option>
						</select>
					</span>
					<span class="form" style="margin-left: 25px;"><input type="text" name="schDcuText" id="schDcuText" value="${vo.schDcuText}" enterSearch data-button="#searchBtn"/></span>
				</div>
				<div class="btn">
		...
		<form name="frm" id="frm" method="post" onsubmit="return false">
			...
			<table class="table-style1" style="margin-bottom: 5px;">
				...
				<tbody id="tby1">
					...
					<c:if test="${libraryList.size() > 0 }">
						<c:forEach items="${libraryList }" var="result">
							<tr >
