		...
		$.ajax({
			...
			success : function(json) {
				...
				if( 0 == json.list.length ) {
					html = '<tr><td colspan="7" rowspan="' + json.vo.pageRows + '" >데이터가 없습니다.</td></tr>';
				} else {
					$.each(json.list, function(i, d) {
						html += '<tr onclick="dtl1(\''+ d.smsNo +'\');">';
						html += '<td>' + d.dbInsDt + '</td>';
						html += '<td>' + d.sendDt + '</td>';
						html += '<td>' + d.subject + '</td>';
						html += '<td>' + d.sendList + '</td>';
						html += '<td>' + d.cntn + '</td>';
						html += '<td>' + d.sendResult + '</td>';
						html += '<td>' + d.regId + '</td>';
						html += '</tr>';
					});
				}
...
<div class="box-style1 x-scroll-auto" >
	...
	<form name="schForm" id="schForm" action="/smsDtl.do" method="post" >
	...
	<table class="table-style1" style="margin-bottom:5px;" >
		...
		<thead>
			<tr>
				...
				<th scope="col">발송이유</th>
				<th scope="col">발송전화번호목록</th>
				<th scope="col">내용</th> 
