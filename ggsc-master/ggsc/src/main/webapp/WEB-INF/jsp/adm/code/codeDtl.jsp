...
<script type="text/javascript">
	...
	function initInput(){
		...
		$("#comNo").val( '<c:out value="${vo.comNo}" />' );
			...
			$.ajax({
				...
				success : function(json) {
					$.each(json.list, function(i, d) {
						...
						$('#comCd').val(d.comCd) ;
						$('#comNm').val(d.comNm) ;
						$('#comGrpCd').val(d.comGrpCd) ;
						$('#comGrpNm').val(d.comGrpNm) ;
						$('#cntn').val(d.cntn) ;
						$('#odr').val(d.odr) ;
						$('#useYn').val(d.useYn) ;
...
<form name="inputForm" id="inputForm" method="post" action='/codeList.do'>
	...
	<input type='hidden' id='comNo' name='comNo' value='${vo.comNo}' />
	...
	<table class="table-style1" style="margin-bottom: 5px;">
		...
		<tbody>
			...
			<tr>
				<td>코드</td>
				<td><input type='text' id='comCd' name='comCd' /></td>
				<td>코드명</td>
				<td><input type='text' id='comNm' name='comNm' /></td>
				<td>코드그룹</td>
				<td><input type='text' id='comGrpCd' name='comGrpCd' /></td>
				<td>코드그룹명</td>
				<td><input type='text' id='comGrpNm' name='comGrpNm' /></td>
			</tr>
