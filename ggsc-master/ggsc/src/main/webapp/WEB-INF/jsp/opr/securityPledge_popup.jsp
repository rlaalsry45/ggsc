...
<script type="text/javascript">
	...
	function fn_reg() {
		...
		if(confirm("보안서약서를 등록하시겠습니까?")) {
			...
			$.ajax({
				type : "POST",
				url : "/gnoincoundb/securityPledge_reg_ajax.do",
				data : param,
				dataType : "json",
...
<section>
	...
	<form id="frm" name="frm" method="post">
		...
		<input type="hidden" name="cnsleId" value="${detail.userId}" />
		...
		<div style="border-style: solid; border-width: thin; width:700px; height: auto; padding: 10px 20px 20px 20px; margin: 10px 10px 10px 10px;">
			...
			<input type="text" id="day" name="day" style="width: 30px;" maxlength="2" oninput="this.value = this.value.replace(/[^0-9]/g, ''); $('#wDay').val(this.value);" /><font style="font-size: large;">일부로 노인상담사업을 수행함에 있어 다음 사항을</font>
			<br><br>
			...
			<br><br>
			<font style="font-size: large; margin-left: 120px;">직&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;위:</font><input type="text" id="rank" name="rank" style="width: 200px; margin-left: 7px;" oninput="this.value = this.value.replace(/[^ㄱ-힣a-zA-Z]/, '');"/>
			<br><br>
			...
			<br><br>
			<font style="font-size: large; margin-left: 120px;">주민등록번호:</font><input type="text" id="jumin1" name="jumin1" style="width: 100px; margin-left: 7px;" maxlength="6" oninput="this.value = this.value.replace(/[^0-9]/g, '');" /> - <input type="password" id="jumin2" name="jumin2" style="width: 100px;" maxlength="7" oninput="this.value = this.value.replace(/[^0-9]/g, '');" />
		</div>
	</form>
	</div>
	<div style="float:right;margin-right:7px;">
		<button type="button" class="btn-basic" id="sBtn" onclick="javascript:fn_reg();">저장</button>
