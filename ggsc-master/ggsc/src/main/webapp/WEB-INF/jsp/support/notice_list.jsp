...
<script type="text/javascript">
	...
	// 임시용
	function fn_gallery(mnuCd){
		document.location.href = "/gnoincoundb/galleryList.do?mnuCd=" + mnuCd;
	}
	
	function fn_detail(num){
		document.location.href = "/gnoincoundb/noticeDtl.do?num=" + num + "&mnuCd=${mnuCd}";
	}

	function fn_search(mnuCd) {
		document.searchForm.action = "/gnoincoundb/noticeList.do?mnuCd=" + mnuCd;
       	document.searchForm.submit(); 
	}
	
	function list(curPage) {
		$("#currentPageNo").val(curPage);
		document.searchForm.action = "/gnoincoundb/noticeList.do";
       	document.searchForm.submit();
	}
...
<section id="content">
	<h2 class="h2-title"><i class="fa fa-check-square"></i>공지사항</h2>
		...
		<form name="searchForm" id="searchForm" method="get" onsubmit="return false">
			...
			<div class="search-box">
				<div class="search-group">
					<span style="padding-left: 100px;">등록일</span> <input type="text" name="schStartDate" id="datepicker1" value="${vo.schStartDate }" style="width: 100px;" readOnly/> &nbsp;&nbsp;&nbsp; ~ &nbsp;<input type="text" name="schEndDate" id="datepicker2" value="${vo.schEndDate}" style="width: 100px;" readOnly/>
					<span class="label">
						<select id="schType" name="schType" style="margin-left: 20px; font-size: 12px;">
		...
		<div class="paginate" id="page1">
			...
			<%-- <button type="button" class="btn-basic" onClick="javascript:fn_gallery('${mnuCd}');" style="background-color:green; color:white;">갤로그</button> --%>
		</div>
		<!-- end -->
	</section>
