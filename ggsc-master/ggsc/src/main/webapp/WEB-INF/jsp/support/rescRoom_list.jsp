...
<script type="text/javascript">
	...
	function fn_detail(num){
		document.location.href = "/gnoincoundb/rescRoomDtl.do?num=" + num + "&mnuCd=${mnuCd}";
	}

	function fn_search(mnuCd) {
		document.searchForm.action = "/gnoincoundb/rescRoomList.do?mnuCd=" + mnuCd;
       	document.searchForm.submit(); 
	}
	
	function list(curPage) {
		$("#currentPageNo").val(curPage);
		document.searchForm.action = "/gnoincoundb/rescRoomList.do";
       	document.searchForm.submit();
	}
...
<section id="content">
	<h2 class="h2-title"><i class="fa fa-check-square"></i>자료실</h2>
				...
				<div class="search-group">
					...
					<span class="label">
						<select id="schType" name="schType" style="margin-left: 20px;">
							...
							<option value="4" <c:if test="${vo.schType==4}">selected</c:if>>작성자</option>
						</select>
					</span>
		...
		<form name="frm" id="frm" method="post" onsubmit="return false">
			...
			<table class="table-style1" style="margin-bottom: 5px;">
				...
				<tbody id="tby1">
					<c:if test="${rescList.size() == 0 }">
						<tr>
							<td colspan="5">정보가 없습니다.</td>
						</tr>
					</c:if>
					<c:if test="${rescList.size() > 0 }">
						<c:forEach items="${rescList }" var="result">
							<tr>
